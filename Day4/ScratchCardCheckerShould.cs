...
public class ScratchCardCheckerShould
{
    ...
    [Test]
    public void Count_The_Number_Of_Cards()
    {
        var scratchCard = """
                          Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
                          Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19
        ...
        var cards = ScratchCardChecker.CountCards(scratchCard);
        
        Assert.That(cards, Is.EqualTo(30));
    }
    ...
    {
        var points = ScratchCardChecker.CountCards(Part1Input.Input);
...
public static partial class ScratchCardChecker
{
    ...
    public static int CountCards(string scratchCardStrings)
    {
        var scratchCards = SplitOnNewLine(scratchCardStrings).Select(OfScratchCard).ToList();
        var numberOfCards = scratchCards.Count;
        var cards = Enumerable.Repeat(1, numberOfCards).ToArray();
        for (var card = 0; card < numberOfCards; card++)
        {
            var scratchCard = scratchCards[card];
            for (var additionalCards = 1; additionalCards <= FromNumberOfWinningNumbers(scratchCard); additionalCards++)
            {
                cards[card + additionalCards] += cards[card];
            }
        }
